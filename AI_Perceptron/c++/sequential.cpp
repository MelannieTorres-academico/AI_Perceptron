...
*  ./a.out < ./tests/ruben_or.txt -true
*  ./a.out < ./tests/ruben_linearly_separable.txt
*  ./a.out < ./tests/breast_cancer.txt
...
using namespace std;
    ...
    double test(int d, long double **x_test_set, long double *y_test_set, int n_size, double *weights, bool show){
      Timer t;
      int i, j;
      ...
      ms += t.stop();
      cout << "Sequential C++ avg time = " << (ms) << " ms\n" << endl;
      return ms;
    }


    double tbb_test(int d, long double **x_test_set, long double *y_test_set, int n_size, double *weights, bool show){
      Timer t;
      int i, j;
      double ms = 0, acum, y_hat;
      t.start();

        #pragma omp parallel for private(acum, y_hat)
        for (i = 0; i < n_size; i++) {
          acum = 0;
          for (j = 0; j < (d+1); j++) {
            acum += x_test_set[i][j] * weights[j];
  ...
  int main (int argc, char* argv[]) {
      ...
      if (errors != 0){
        ...
        bool flag = (argc == 2);
        double seq = test(d, x_test_set, y_test_set,  n_size, weights, flag);
        double tbb = tbb_test(d, x_test_set, y_test_set,  n_size, weights, flag);

        cout<<"\nSpeed up: "<<(seq/tbb)<<endl;
      }
